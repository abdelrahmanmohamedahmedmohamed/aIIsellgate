...
let productDescription =
  document.getElementsByClassName("productDescription")[0];
...
let theSallerInfoNamePic = document.getElementsByClassName(
  "theSallerInfoNamePic"
)[0];
...
let product = {};
if (parId !== null) getProduct();

function getProduct() {
  fetch(
    `https://cros-anywhere.herokuapp.com/https://sellgate1.herokuapp.com/getSpecialPru?id=${parId}`
  )
...
function displayProduct() {
  ...
  document.getElementsByClassName("img-select")[0].innerHTML =
    imgSelectContainer;
  ...
  document.getElementsByClassName("img-showcase")[0].innerHTML =
    imgShowcaseContainer;
...
let addCommentBtn = document.getElementsByClassName("btnAddComment")[0];
if (addCommentBtn !== undefined) {
...
function getComments() {
  fetch(
    `https://cros-anywhere.herokuapp.com/https://sellgate1.herokuapp.com/getReview?id=${parId}`
  )
    ...
    .then(function (getCommentsData) {
      ...
      if (comments.length == 0) {
        document.getElementsByClassName(
          "CustReviewsArea"
        )[0].innerHTML = `<div class="alert alert-warning ErrorCommntSho" role="alert">
      No Comments To Show...
                  </div>`;
      } else {
        displayComments();
...
function getRecommend() {
  fetch(
    `https://cros-anywhere.herokuapp.com/https://sellgate1.herokuapp.com/getrecommendMachine?id=${product.spe_id}`
  )
    .then((data) => {
      return data.json();
    })
